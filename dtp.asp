...
function init(d_start,d_end)
{
     ...
     temp_str+="<font font-size: 9pt; color=red>日</font></td><td>";temp_str+="<font font-size: 9pt>一</font></td><td>"; temp_str+="<font font-size: 9pt>二</font></td><td>"; temp_str+="<font font-size: 9pt>三</font></td><td>"
     temp_str+="<font font-size: 9pt>四</font></td><td>";temp_str+="<font font-size: 9pt>五</font></td><td>"; temp_str+="<font font-size: 9pt>六</font></font></td></tr>";
     for (i=1 ;i<=6 ;i++)
     {
     temp_str+="<tr>";
        for(j=1;j<=7;j++){
...
function set_cele_date(year,month)
{
   ...
   for (i=1;i<=6;i++)
      ...
      eval("c"+i+"_"+j+".bgColor=\"#FFFFFF\"");//日的背景色
   ...
   while(month-1==nd.getMonth())
    { j=(nd.getDay() +1);
      ...
      if ((nd.getDate()==today.getDate())&&(cele_date_month.value==today.getMonth()+1)&&(cele_date_year.value==today.getYear())){
      	 eval("c"+p+"_"+j+".bgColor=\"#FFFFFF\"");//今天的日
...
g_object.fireEvent("onChange");//有一个input,是放时间的,如:1999-12-03,但不能手工输入它的值,因为我把它设为了readonly 。它有一个onClick事件,点它时,弹出一个对话框,就可选中它的值了,但是改变之后，却不能击活onChange事件。而我又非常希望用到这个onChange事件，请问，这是怎么回事？我应该怎么办？谢谢！
                               //在你給它賦值的時候呼叫onChange事件不就行了嗎
                               //input1.value="xxx-xx-xx";
                               //input1.fireEvent("onChange");
                               //hjh20040619引用“http://community.csdn.net/Expert/topic/2657/2657873.xml?temp=.1257898”
};
